...
htcl — "holistic Tcl" — is a structured dialect of Tcl for driving EDA tools.
It is the language `vw` uses to configure IP, build block designs, and run the
Vivado flow, and it exists to make those interfaces documented, typed, and
checkable before a build runs rather than hours into one.

This document covers why htcl exists, the whole of the language, where it
differs from Tcl, and how to check a program with `vw`.
...
related helpers under a `<ns>::<name>` prefix — see Namespaces below.
...
slot (Return types) — primitives (`string`, `int`, `bool`, `unit`),
...
### What htcl restricts or interprets differently
...
## Checking an htcl program with `vw`
...
### What `vw check` does *not* enforce
