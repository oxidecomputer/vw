...
use crate::VwError;
use crate::{
    nvc_helpers::{run_nvc_elab, run_nvc_sim},
    RecordProcessor, VhdlStandard,
};

use proc_macro2::TokenStream;
use quote::{format_ident, quote};

use vhdl_lang::ast::{AbstractLiteral, Expression, Literal};
...
pub async fn anodize_records(
    ...
    for name in &processor.tagged_names {
        match processor.symbols.get(name) {
            Some(VwSymbol::Record(record)) => {
                ...
                let pkg_name =
                    record.get_pkg_name().ok_or_else(|| VwError::CodeGen {
                        message: format!(
                            "Serialization not supported for \
                        records not in packages. Record : {:}",
                            record.get_name()
                        ),
                    })?;
                ...
                let file_data = processor
                    .file_info
                    .get(record_filename)
                    .ok_or(VwError::CodeGen {
                        message: format!(
                            "Somehow couldn't find information for file {:}",
                            record_filename
                        ),
                    })?;
            ...
            None => {
                return Err(VwError::CodeGen {
                    message: format!(
                        "Tagged type with name {:} not found",
                        name
                    ),
                })
            }
    ...
    let mut expr_to_resolve: HashMap<String, Vec<ConstraintID>> =
        HashMap::new();
    ...
    let tb_files: Vec<String> = referenced_files
        .iter()
        .cloned()
...
fn process_sim_output(
    ...
    for line in stdout_str.lines() {
        ...
        let index: usize = parts[0]
            ...
            .unwrap()
            .parse()
            .map_err(|e| VwError::CodeGen {
                message: format!(
                    "Somehow generated an unparseable simulation output : {:}.\
                 Look at sim.out",
                    e
                ),
            })?;
        let value: usize = parts[1].parse().map_err(|e| VwError::CodeGen {
            message: format!("Expression couldn't be evaluated : {}", e),
        })?;

        let key = &expr_keys[index];
        let constraint_ids =
            exprs_to_resolve.get(key).ok_or_else(|| VwError::CodeGen {
                message: format!(
                    "Somehow got expression {:} which doesn't exist",
                    key
                ),
            })?;
        for id in constraint_ids {
            ...
            if let Some(bitfield) = &mut field.bit_width {
                ...
                return Err(VwError::CodeGen {
                    message: format!(
                        "Somehow tried to generate an expression for \
