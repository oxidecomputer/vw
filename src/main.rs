...
async fn main() {
    ...
    match cli.command {
        ...
        Commands::Add { repo, branch, commit, src, name } => {
            if let Err(e) = add_dependency(repo, branch, commit, src, name).await {
                eprintln!("{} {e}", "Error:".bright_red());
                process::exit(1);
            }
        }
...
async fn update_workspace() -> Result<()> {
    ...
    for (name, dep) in &config.dependencies {
        ...
        let commit_sha = resolve_dependency_commit(&dep.repo, &dep.branch, &dep.commit)
            .await
            .with_context(|| format!("Failed to resolve commit for dependency '{name}'"))?;
        ...
        if !dep_path.exists() {
            ...
            download_dependency(&dep.repo, &commit_sha, &dep.src, &dep_path)
                .await
                .with_context(|| format!("Failed to download dependency '{name}'"))?;
        } else {
            println!("Using cached version of {} at {}", name.cyan(), commit_sha.cyan());
        }
        
        lock_file.dependencies.insert(name.clone(), LockedDependency {
            repo: dep.repo.clone(),
        ...
        if !vhdl_files.is_empty() {
            let portable_files = vhdl_files.into_iter()
                .map(|path| make_path_portable(path))
                .collect();
            vhdl_ls_config.libraries.insert(name.clone(), VhdlLsLibrary {
...
fn init_workspace(name: String) -> Result<()> {
    ...
    if config_path.exists() {
        return Err(anyhow!("{} vw.toml already exists in current directory", "✗".bright_red()));
    }
    ...
    println!("{} Initialized workspace: {}", "✓".bright_green(), name.cyan());
    
    Ok(())
}

async fn add_dependency(repo: String, branch: Option<String>, commit: Option<String>, src: Option<String>, name: Option<String>) -> Result<()> {
    let mut config = load_workspace_config().unwrap_or_else(|_| {
        WorkspaceConfig {
            workspace: WorkspaceInfo { name: "workspace".to_string(), version: "0.1.0".to_string() },
            dependencies: HashMap::new(),
        }
    });
    ...
    if branch.is_none() && commit.is_none() {
        return Err(anyhow!("{} Must specify either --branch or --commit", "✗".bright_red()));
    }
...
fn remove_dependency(name: String) -> Result<()> {
    ...
    if config.dependencies.remove(&name).is_some() {
        ...
        return Err(anyhow!("{} Dependency '{}' not found", "✗".bright_red(), name));
    }
...
fn clear_cache() -> Result<()> {
            ...
            for entry in entries.flatten() {
                ...
                if let Some(file_name_str) = file_name.to_str() {
                    if file_name_str.starts_with(&format!("{name}-")) {
                        ...
                        if dep_path.is_dir() {
                            println!("Removing cached dependency: {}", file_name_str.cyan());
    ...
    if cleared_count > 0 {
        println!("{} Cleared {} cached repositories", "✓".bright_green(), cleared_count);
    } else {
...
fn load_workspace_config() -> Result<WorkspaceConfig> {
    let config_path = Path::new("vw.toml");
    if !config_path.exists() {
        return Err(anyhow!("{} No vw.toml file found in current directory", "✗".bright_red()));
    }
    
    let config_content = fs::read_to_string(config_path)
        .context("Failed to read vw.toml")?;
    
    let config: WorkspaceConfig = toml::from_str(&config_content)
        .context("Failed to parse vw.toml")?;
...
fn get_deps_directory() -> Result<PathBuf> {
    let home_dir = dirs::home_dir()
        .ok_or_else(|| anyhow!("{} Could not determine home directory", "✗".bright_red()))?;
...
async fn resolve_dependency_commit(repo_url: &str, branch: &Option<String>, commit: &Option<String>) -> Result<String> {
    match (branch, commit) {
        (Some(_), Some(_)) => {
            Err(anyhow!("{} Cannot specify both branch and commit for dependency", "✗".bright_red()))
        },
        (None, None) => {
            Err(anyhow!("{} Must specify either branch or commit for dependency", "✗".bright_red()))
        },
        (None, Some(commit)) => Ok(commit.clone()),
        (Some(branch), None) => {
            get_branch_head_commit(repo_url, branch).await
        }
    }
}

async fn get_branch_head_commit(repo_url: &str, branch: &str) -> Result<String> {
    ...
    if !output.status.success() {
        ...
        return Err(anyhow!("{} Git ls-remote failed: {}", "✗".bright_red(), stderr));
    }
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    let commit = stdout.split_whitespace()
        .next()
        .ok_or_else(|| anyhow!("{} Could not parse git ls-remote output", "✗".bright_red()))?;
    
    Ok(commit.to_string())
}

async fn download_dependency(repo_url: &str, commit: &str, src_path: &str, dest_path: &Path) -> Result<()> {
    ...
    if !clone_output.status.success() {
        ...
        return Err(anyhow!("{} Git clone failed: {}", "✗".bright_red(), stderr));
    }
    ...
    if !checkout_output.status.success() {
        ...
        return Err(anyhow!("{} Git checkout failed: {}", "✗".bright_red(), stderr));
    }
    
    let src_dir = temp_dir.path().join(src_path);
    if !src_dir.exists() {
        return Err(anyhow!("{} Source path '{}' does not exist in repository", "✗".bright_red(), src_path));
    }
...
fn find_vhdl_files_recursive(dir: &Path, vhdl_files: &mut Vec<PathBuf>) -> Result<()> {
...
fn write_vhdl_ls_config(managed_config: &VhdlLsConfig) -> Result<()> {
    ...
    for (name, library) in &managed_config.libraries {
        existing_config.libraries.insert(name.clone(), library.clone());
    }
